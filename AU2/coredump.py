...
def write_coredump(exception: Exception) -> str:
    ...
    try:
        from AU2 import BASE_WRITE_LOCATION, __version__
    except Exception:
    ...
    txt_path = str(Path(BASE_WRITE_LOCATION) / f"coredump_{stamp}.txt")
    pkl_path = str(Path(BASE_WRITE_LOCATION) / f"coredump_{stamp}.pkl")
