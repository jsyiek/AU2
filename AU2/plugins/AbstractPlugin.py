...
class Export:
    ...
    def __init__(self, identifier: str, display_name: str, ask, answer,
        ...
        Args:
            ...
            ask: function or tuple of functions that generate(s) a list of HTML components. The functions take the
                output of the `options_functions` as output. In the case of a tuple, the functions after the first one
                take as their argument a dictionary arg -> str representing the output of the components generated by
                the previous `ask` functions; the first `ask` function however takes as arguments the selections made
                in the `options_functions` stage.
